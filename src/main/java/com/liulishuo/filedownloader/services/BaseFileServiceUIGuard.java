...
public abstract class BaseFileServiceUIGuard<CALLBACK extends Binder, INTERFACE extends IInterface>
    ...
    public void bindStartByContext(final Context context, final Runnable connectedRunnable) {
        ...
        if (runServiceForeground) {
            if(!FileDownloadUtils.canStartForgroundService(context)) {
                FileDownloadLog.d(this, "We need to start a foreground service but in android api 31+ can't start a foreground service from background, so cancel the current operation...");
                return;
            }
            if (FileDownloadLog.NEED_LOG) FileDownloadLog.d(this, "start foreground service");
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) context.startForegroundService(i); //todo: check for fg service
        } else {
